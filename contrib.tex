...
in the view of faster curve arithmetic.

\paragraph{Cofactor.}
While the existence of a very small torsion subgroup
may lead to some side-channel attacks,
it is a necessary condition for availability
of some faster curve coordinates such as Edwards or Montgomery curves.
...
twist-prime curves are themselves exceptional in some way.
We do not think that this danger
...
Nonetheless, the usual bound chosen for these criteria are met
by a randomly chosen curve with overwhelming probability.
Therefore, we included those criteria
in the “normality” section (\ref{ss:normal}) below
and their abscence here is not problematic.
...
It is possible to perform index calculus on a lift of the curve.
Nonetheless, in the current state of art, it is more expensive
...
such as the NIST elliptic curves~\cite{2000fips186-2},
