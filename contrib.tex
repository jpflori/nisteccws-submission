...
We recall that security is the primary matter,
while speed is another important one,
but both cannot be optimally achieved in the current state-of-the-art:
...
where $N/c$~is a composite number,%a number such that~$N < 2 (√p - 1)^2$,
...
and so on its endomorphism ring~$\ro O_E$.
